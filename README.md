# 👋 Hi, I'm Wong

🚀 Backend & Data Engineer | AI Systems Builder  
⚡ I design and build scalable systems — from real-time pipelines to intelligent APIs

---

## 🧠 What I Do

- ⚙️ Build **data-intensive systems** (ETL pipelines)
- 🧩 Design **backend architectures** with FastAPI / NestJS
- 🧠 Experiment with **AI**
- 🚀 Turn complex ideas into **production-ready tools**

---

## 🛠 Tech Stack

### Backend

<p>
  <img src="https://skillicons.dev/icons?i=nestjs,fastapi,postgres,nodejs" />
</p>

### Frontend

<p>
  <img src="https://skillicons.dev/icons?i=react,vue" />
</p>

### ☁️ DevOps & Infrastructure

<p>
  <img src="https://skillicons.dev/icons?i=docker,ansible,git" />
</p>

### 🤖 AI / ML

<p>
  <img src="https://skillicons.dev/icons?i=pytorch,tensorflow" />
</p>

---

## 📈 GitHub Stats

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=wongty27&show_icons=true&theme=catppuccin_mocha)

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=wongty27&layout=compact)

---

## 🧭 Current Focus

- ⚡ Real-time systems & CDC pipelines
- 🕸 Graph-based analytics
- 🧠 AI-native backend architectures
